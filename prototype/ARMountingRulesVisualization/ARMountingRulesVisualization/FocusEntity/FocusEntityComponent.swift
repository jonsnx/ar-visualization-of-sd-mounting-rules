...
public struct FocusEntityComponent: Component {
    ...
    public enum Style {
        ...
        case custom(
            onColor: MaterialColorParameter,
            offColor: MaterialColorParameter,
            mesh: MeshResource = MeshResource.generateCylinder(height: 0.05, radius: 0.05)
        )
    }
    ...
    var customStyle: CustomStyle? {
        switch self.style {
        case .custom(let onColor, let offColor, let mesh):
            return CustomStyle(
                onColor: onColor, offColor: offColor, mesh: mesh
            )
        default:
            return nil
        }
    ...
    public static let detector = FocusEntityComponent(style: .custom(onColor: .color(.green), offColor: .color(Material.Color.red.withAlphaComponent(0.2)), mesh: FocusEntityComponent.defaultDetector))
    ...
    static var defaultDetector = MeshResource.generateCylinder(
