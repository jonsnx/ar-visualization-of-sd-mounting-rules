...
Zur Validierung dieser Regeln existiert ein Framework, das die Überprüfung der Montageregeln übernimmt. Eine zentrale Komponente dieses Frameworks ist der \texttt{MountingRuleManager}. Diese Klasse verwaltet eine Liste von Montageregeln und stellt sicher, dass die festgelegten Anforderungen eingehalten werden.
...
Der \texttt{SmokeDetector} kann in der AR-Szene platziert werden, sobald die Focus-Entity eine gültige Position aufweist. Eine Benutzerinteraktion mit einer Schaltfläche löst die Platzierung des Rauchmelders aus, wobei die Position der Focus-Entity als Ankerpunkt für den Rauchmelder verwendet wird. Die Bestimmung der Abstände zu umliegenden Wänden wird wie folgt implementiert \cite{appledevdoc}:
